<div class="col-sm-12">
    <div class="well">
        <h5><a href="index.php?page=lpnbp">Realisasi PNBP</a> /
            Direktorat Jenderal Pembiayaan Perumahan</h5>      
    </div>
                ...
                <div class="col-sm-6">
                    <select class="form-control" id="nmsatker" name="kdsatker">
                        <option selected="selected">--Pilih Satuan Kerja--</option>
                            ...
                            LEFT JOIN r_satker_2017 AS S ON S.kdunit = U.kdunit WHERE S.kdunit = '08' AND S.kdaktif = 1 ORDER BY nmsatker";
                ...
                <?php
                    ...
                    if(!isset($_POST['cari'])){
                        ...
                        $query  = mysqli_query($konek, "SELECT U.kdunit, S.kdsatker, D.wbws, D.kode_klus, D.kode_billing, D.kode_bp, D.NTPN, D.ntbntp, D.kode_uf, D.jml_setoran, D.tanggal
                            ...
                            LEFT join r_unit_2018 AS U on S.kdunit = U.kdunit
                            LEFT JOIN d_simponi_2017 as D on D.kdsatker = S.kdsatker
                            WHERE S.kdaktif = 1 AND D.kdunit = 08 AND D.kode_uf = '$jenis' AND S.kdsatker = '$kdsatker' AND D.tanggal BETWEEN '$tanggalawal' AND '$tanggalakhir' OR S.kdaktif = 1 AND D.kdunit = 08 AND D.kode_uf = '$jenis' OR S.kdaktif = 1 AND D.kdunit = 08 AND S.kdsatker = '$kdsatker' OR S.kdaktif = 1 AND D.kdunit = 08 AND D.tanggal BETWEEN '$tanggalawal' AND '$tanggalakhir'
                            GROUP BY D.tanggal")or die(mysqli_error($konek));
                                if(mysqli_num_rows($query) == 0){
                                    echo '<tr><td collspan="4" align="center">Tidak ada data!</td></tr>';
                                }
                        ...
                        <?php
                            ...
                            if(!isset($_POST['cari'])){
                                $querytampil = mysqli_query($konek, "SELECT SUM(D.jml_setoran) as jumlah
                                    ...
                                    LEFT JOIN d_simponi_2017 AS D ON D.kdsatker = S.kdsatker
                                    WHERE S.kdunit = 08 AND S.kdaktif = 1")  or die(mysqli_error($konek));
                                ...
                                $querytampil = mysqli_query($konek, "SELECT SUM(D.jml_setoran) as jumlah
                                    ...
                                    LEFT JOIN r_satker_2017 AS S ON S.kdunit = U.kdunit
                                    LEFT JOIN d_simponi_2017 AS D ON D.kdsatker = S.kdsatker
                                    WHERE S.kdunit = 08 AND S.kdaktif = 1 AND D.kode_uf = '$jenis' AND S.kdsatker = '$kdsatker' AND D.tanggal BETWEEN '$tanggalawal' AND '$tanggalakhir'")  or die(mysqli_error($konek));
                                $data = mysqli_fetch_array($querytampil);
                                echo "".$data['jumlah']."";
                            }
